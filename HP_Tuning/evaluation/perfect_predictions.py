...
def check_predictions(folder):
    ...
    f = FileManager(os.path.join(folder, prediction_file))
    # f.open_file_txt_no_codecs("r")
    predictions = f.read_file_txt()

    print("READ {} PRED".format(len(predictions)))

    f.close_file()

    f = FileManager(os.path.join(folder, prediction_file.replace("predictions", "targets")))
    # f.open_file_txt_no_codecs("r")
